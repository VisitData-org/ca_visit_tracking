...
function seriesToPlot(stateOrCounty) {
  ...
  if (weatherDataCheck) {
    ...
    _.each(results, function (fields) {
      _.each(fields.data, function (formatted) {
    ...
    //  the weather of the selected counties
    countiesWeather = _.intersection(
    ...
    _.each(countiesWeather, function (county) {
      var arrTemp = [];
      var arrPrec = [];
      var dataTemp = null;
      var dataPrec = null;
      ...
      //array dates/weather
      _.each(dates[county], function (timeStamp) {
      ...
      if (dataTemp && dataPrec) highchartsWeatherData.push(dataTemp, dataPrec);
    });
    results = _.union(results, highchartsWeatherData);
  }
